...
## 关键发现 / Hidden Facts That Mattered
...
More precisely, `bare OpenCode + Librarian` only participated in the last two rounds, but in both `T2` and `T3` it matched or outperformed `omo` in research quality while using only `1/6` to `1/2` of the tokens.
